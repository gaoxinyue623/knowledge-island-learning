...
} from '@/data/curriculum/grade-2/chinese-pep-upper'
import type {
  Challenge,
...
/**
 * Small, self-authored practice prompts for the Grade 2 upper-volume lessons.
 * They stay in the development candidate dataset until independently reviewed.
 */
export const G2_PEP_CHINESE_S1_ORIGINAL_EXERCISE_SOURCE_ID =
  'G2_PEP_CHINESE_S1_ORIGINAL_EXERCISES_V1'

const generatedAt = '2026-09-04T00:00:00.000Z'
const baseRecord = {
  sourceId: G2_PEP_CHINESE_S1_ORIGINAL_EXERCISE_SOURCE_ID,
...
export const candidateG2ChineseContentExpansionBundles: readonly ContentExpansionBundle[] =
  gradeTwoChineseUpperLessons.map((lesson, index) => {
    const knowledgePoint = gradeTwoChineseUpperKnowledgePoints[index]
    if (!knowledgePoint) throw new Error(`G2_CHINESE_EXERCISE_CONTEXT_MISSING:${index + 1}`)
    const unit = gradeTwoChineseUpperCurriculum.units.find((item) => item.id === lesson.unitId)
    if (!unit) throw new Error(`G2_CHINESE_EXERCISE_UNIT_MISSING:${lesson.unitId}`)

    const title = displayTitle(lesson.title)
    return {
      knowledgePointId: knowledgePoint.id,
      textbookId: G2_PEP_CHINESE_S1_TEXTBOOK_ID,
      unitId: unit.id,
      lessonId: lesson.id,
