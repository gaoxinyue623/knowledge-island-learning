...
/**
 * Original practice prompts aligned to the Shenzhen Grade 2 English upper
 * volume. Candidate records remain outside production until reviewed.
 */
export const G2_SHENZHEN_ENGLISH_S1_ORIGINAL_EXERCISE_SOURCE_ID =
  'G2_SHENZHEN_ENGLISH_S1_ORIGINAL_EXERCISES_V1'

const generatedAt = '2026-09-04T00:00:00.000Z'
const baseRecord = {
  sourceId: G2_SHENZHEN_ENGLISH_S1_ORIGINAL_EXERCISE_SOURCE_ID,
...
export const candidateG2ShenzhenEnglishUpperContentExpansionBundles: readonly ContentExpansionBundle[] =
  gradeTwoShenzhenEnglishUpperLessons.map((lesson, index) => {
    const definition = gradeTwoShenzhenEnglishUpperLessonPracticeDefinitions[index]
    const knowledgePoint = gradeTwoShenzhenEnglishUpperKnowledgePoints[index]
    if (!definition || !knowledgePoint)
      throw new Error(`G2_SHENZHEN_ENGLISH_S1_EXERCISE_CONTEXT_MISSING:${index + 1}`)
    const unit = gradeTwoShenzhenEnglishUpperCurriculum.units.find(
      (item) => item.id === lesson.unitId,
    )
    if (!unit) throw new Error(`G2_SHENZHEN_ENGLISH_S1_EXERCISE_UNIT_MISSING:${lesson.unitId}`)
    ...
    return {
      ...
      textbookId: G2_SHENZHEN_ENGLISH_S1_TEXTBOOK_ID,
      unitId: unit.id,
      lessonId: lesson.id,
