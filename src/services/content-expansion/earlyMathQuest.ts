...
export function createEarlyMathQuest(bundle: ContentExpansionBundle): ReadingQuest | null {
  ...
  const setStartUnitHints = (guidanceByTitle: Record<string, [string, string]>) => {
    for (const stage of quest.stages) {
      const guidance = guidanceByTitle[stage.title]
      if (!guidance) throw new Error(`Missing start-unit guidance for ${stage.title}`)
      stage.hint = guidance[0]
      stage.hints = guidance
    }
  }
  ...
  if (family === 'welcome') {
    ...
    setStartUnitHints({
      准备出发: [
        '观察是谁正在介绍，先想什么做法能让同伴把话说完。',
        '把选项分成“认真听”和“打断别人”两类，再选前一类。',
      ],
      想一想: [
        '观察题目是在问发言前的动作，先找需要等待的时刻。',
        '先做让老师看见的准备动作，再等轮到自己。',
      ],
      看图发现: [
        '观察每个圆点代表什么，用手指逐个点着数。',
        '点过一个就移开手指，检查有没有漏掉或重复。',
      ],
      配对小能手: [
        '观察文具和家具各自用来做什么，再找用途相配的一对。',
        '先从最熟悉的物品开始，配好后说说它为什么适合。',
      ],
      排排队: [
        '观察准备上课要用到的物品和动作，想想先后顺序。',
        '先完成能开始下一步的准备，再检查课本和铅笔是否整齐。',
      ],
      辨一辨: [
        '观察题目问的是倾听的作用，回想听同伴说话时能发现什么。',
        '用自己的课堂经历检验：认真听时，能不能学到新的想法？',
      ],
      生活小挑战: [
        '观察每个做法会不会爱护文具、照顾同伴，再逐项检查。',
        '题目要选两种，先排除会伤害物品或影响别人的做法。',
      ],
      再想一步: [
        '观察哪种开场会让新同伴愿意继续交流。',
        '先想礼貌问候，再想怎样清楚介绍自己；不用急着选。',
      ],
    })
    ...
    if (d.unitIndex === 0 && family === 'count')
      setStartUnitHints({
        准备出发: [
          '观察图里要数的物品，手指逐个点着数。',
          '每点一个就说一个数字，最后一个数字表示总数。',
        ],
        想一想: [
          '观察怎样才不会把同一件物品数两次。',
          '数过的物品可以在心里做记号，再继续数下一个。',
        ],
        看图发现: [
          '观察相邻的数字，先顺着数一数再填空。',
          '从前一个数字开始往后接着数一步，检查顺序。',
        ],
        配对小能手: [
          '观察“几个”和数字卡的对应关系，逐组读一读。',
          '先配最确定的一组，再用点数检查其他物品。',
        ],
        排排队: [
          '观察三个数字的大小，先找最小的再往后排。',
          '每放一张卡就和前一张比一比，确保顺序没有倒过来。',
        ],
        辨一辨: [
          '观察物品只是换了摆放位置，数量有没有增加或拿走。',
          '逐个点数两次，比较摆放前后的总数。',
        ],
        生活小挑战: [
          '观察记录卡的一个圆点代表几本书，再按实际数量画。',
          '先说清每个点代表什么，再检查点的个数。',
        ],
        再想一步: [
          '观察题目数的是整盒还是盒里的物品。',
    ...
    if (d.unitIndex === 0)
      setStartUnitHints({
        准备出发: [
          '观察队伍从哪一边开始数，先确定方向。',
          '从起点逐个报位置，数到题目说的第几个再停。',
        ],
        想一想: [
          '观察题目问的是队伍总数还是某个位置。',
          '看到“几个”就逐个数完整队伍，不只看一个位置。',
        ],
        看图发现: [
          '观察这次从右边开始，先把右端当作起点。',
          '从右向左逐个数，第一位就是右端的朋友。',
        ...
        配对小能手: [
          '观察每张卡是在问总数、位置还是方向。',
          '先读清问题里的关键词，再找表达相同意思的搭档。',
        ],
        排排队: [
          '观察题目要求从左到右，按队伍位置排好朋友。',
          '先找到最左边，再顺着方向一个一个放。',
        ],
        辨一辨: [
          '观察“几个”和“第几个”问的是否相同。',
          '一个问总数，一个问位置；分别用题目里的词检查。',
        ],
        生活小挑战: [
          '观察同一位朋友从另一边数时，起点变成哪里。',
          '从右端重新逐个数，找到和原来同一位的朋友。',
        ],
        再想一步: [
          '观察只是换了数的方向，队伍里的朋友有没有增减。',
          '先数完整队伍，再判断总数会不会跟着方向改变。',
        ],
      })
  } else if (family === 'zero') {
