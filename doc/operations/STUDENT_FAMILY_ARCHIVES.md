...
- 家长操作的是完整 snapshot；可在预览上传时绑定云端档案，启用正式 Agent 学习完成后的自动上传（带 revision CAS）。取消勾选并确认保存会关闭绑定；恢复始终复制为新的本机档案，原档案保留。
- 自动上传不包含后台下载或冲突合并；其他学习流程仍需手动上传。部署见 [UCloud 部署与维护](UCLOUD_DEPLOYMENT.md)。
