...
- 二下语文另提供“含《我不是最弱小的》”版本，保留原版本和各自进度，由用户明确选择，不自动迁移；见[本次接入说明](../curriculum/CURRICULUM_G2_CHINESE_LOWER_REVISION.md)。
...
**当前实现（2026-09-06 体验优化后）**：知识点详情首次默认显示「基础热身」，可切换「强化训练」；强度与组号按档案记住。强化训练每组九关，分为独立热身、方法进阶、综合实战；三组固定变式循环，每组进度单独保存，换回旧组可以继续。训练内容和覆盖范围见[课后强化训练](../learning/QUEST_TRAINING.md)。
...
**当前实现（2026-09-07）**：`/thinking-islands` 展示规律发现、逻辑侦探、空间想象、策略工坊四座原创训练岛；`/thinking-islands/:islandId` 展示该岛的训练路线（包含入门、进阶、挑战）；新增联动开关与四宫数独后共十五条路线、六十个任务；`/thinking-islands/:islandId/:missionId` 承载实际游戏。路线可以自主选择，不依赖地区、年级或教材配置。首页与教材地图提供入口。

**交互与状态**：每条路线四个任务，正确完成当前任务后继续。两步提示、重试、撤回或取消选择、小结和重新挑战均由独立游戏组件负责；支持选卡、条件配对、依赖排序、方格路线、材料组合、联动开关及四宫数独。校验接受所有符合规则的顺序、路径和组合；不是限时速度赛或智力测评。

**事实边界**：思维训练不是第四个教材学科，不改变 Curriculum、QuestionSession、Mastery、Strategy、地图解锁、正式历史、错题本或奖励。`thinkingStore` 单独保存当前学习档案的完成任务 ID，刷新恢复到未完成任务；不保存未提交方案或本次提示／重试统计。数据和来源、存储失败行为、验证记录见[思维群岛说明](../learning/THINKING_ISLANDS.md)。
...
**事实与状态边界**：这些内容明确标为原创课外阅读，不是教材原文。阅读内容与练习在独立数据目录中，复用只读展示及纯校验组件，不写正式学习历史、错题本、教材掌握度、地图解锁或奖励。闯关记录按档案与练习保存到本机，刷新后继续，目录显示已通过关卡；未提交草稿和开放表达自查不保存。内容检查、既有教材覆盖范围和验证记录见[阅读群岛与内容结构检查](../learning/READING_ISLANDS.md)。
...
**当前实现**：`/settings` 支持昵称、减少动效、媒体默认静音和默认展开文字讲解的保存。昵称使用 `studentStore`，播放与动效偏好使用 `preferencesStore`；偏好已接入课程、题目媒体和全局动效。帮助区提供课程设置、学习记录、本地保存和网络使用说明；系统推送、后台提醒和全站学习记录跨设备同步未实现；宠物小屋提供单独的账号备份入口。
...
### 学习积分与宠物小屋（2026-09-07）

`/achievements` 已接入免费领养、取名/改名、三种食物兑换与喂养、五阶段成长相册、学习积分概览和收支记录；原教材成长能量及里程碑单独保留。全站页头显示可用积分并链接宠物小屋；该页与思维、阅读一样不强制教材配置。取当前学习档案，事务保存成功才展示兑换或喂养成功。完整规则、数据所有权与失败边界见 [学习积分与宠物成长](../learning/PET_GROWTH.md)。

第二阶段新增三位伙伴收藏及独立经验、四种可兑换家园装饰、切换伙伴查看成长相册；家长报告增加按时间与来源筛选的积分统计。宠物小屋底部折叠区提供家长注册/登录、备份列表、预览和恢复。该入口明确只备份宠物账本，遇到不兼容历史停止覆盖，不宣称全站自动同步。启动与部署见 [宠物后端](../operations/PET_BACKEND.md)。

### 学习空间入口与返回流程（2026-09-07）

`/profile`、`/character`、`/history`、`/wrong-book` 和 `/review-queue` 允许未配置教材的档案打开；实际进入教材课程时仍执行教材配置检查。个人页在没有课程资料时显示当前个人档案，课程名称读取失败也保留功能入口。成长摘要和学习记录通过 useLearningProfile 取同一档案 ID，异步课程名称及题目标题不会覆盖已切换档案的视图。

角色装扮支持预览、保存和返回个人页；它是学习头像，与成长页的宠物收藏独立。学习记录显示可读取的知识点名称，提供回到对应学习的入口；历史页仅列教材课程和练习，阅读与思维参与奖励仍在成长页查看。错题本和待巩固复用已有重练、完成及重新打开操作，空列表新增去学习入口。家长中心使用当前档案的学习报告。五项均提供返回个人空间的链接。

全局路由切换显示加载反馈；目标页面模块加载失败时保留当前页面，并提供指向目标地址的重新打开链接，避免无提示的跳转失败。`tests/learning-space.test.ts` 验证真实目标组件的五入口往返、未配置教材边界、档案切换和加载失败恢复；浏览器逐项实测入口、返回及装扮保存后再次进入的持久化结果。
